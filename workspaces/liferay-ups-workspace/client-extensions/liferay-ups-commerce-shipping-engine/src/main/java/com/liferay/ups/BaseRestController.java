...
public abstract class BaseRestController {
	...
	protected ResponseEntity<String> post(Jwt jwt, String json, Log log)
		...
		Map<String, JSONObject> listTypeEntriesMap = _getListTypeEntriesMap(
			jwt, ratingCodes);
		...
		return new ResponseEntity<>(
			new JSONObject(
				...
				"shippingOptions",
				_toShippingOptionsJSONArray(
					responseJSONArray, listTypeEntriesMap)
			).toString(),
			HttpStatus.OK);
	}
	...
	private Map<String, JSONObject> _getListTypeEntriesMap(
		Jwt jwt, List<String> ratingCodes) {

		Map<String, JSONObject> listTypeEntriesMap = new HashMap<>();
		...
		JSONArray listTypeEntriesJSONArray = jsonObject.getJSONArray("items");

		for (int i = 0; i < listTypeEntriesJSONArray.length(); i++) {
			JSONObject curJSONObject = listTypeEntriesJSONArray.getJSONObject(
				i);

			String externalReferenceCode = curJSONObject.getString(
				"externalReferenceCode");

			String code = StringUtil.removeSubstring(
				externalReferenceCode, "L_UPS_CODE_");

			if (ratingCodes.contains(code)) {
				listTypeEntriesMap.put(code, curJSONObject);
			}
		}

		return listTypeEntriesMap;
	}
	...
	private JSONArray _toShippingOptionsJSONArray(
		JSONArray responseJSONArray,
		Map<String, JSONObject> listTypeEntriesMap) {
		...
		for (int i = 0; i < responseJSONArray.length(); i++) {
			...
			JSONObject listTypeEntryJSONObject = listTypeEntriesMap.get(
			...
			shippingOptionsJSONArray.put(
				new JSONObject(
					...
					"key", listTypeEntryJSONObject.getString("key")
				).put(
					"name", listTypeEntryJSONObject.getString("name")
				).put(
					"priority", i
				));
		}
