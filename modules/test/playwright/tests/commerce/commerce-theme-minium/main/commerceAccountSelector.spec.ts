...
import getPageDefinition
	from "../../../layout-content-page-editor-web/main/utils/getPageDefinition";
...
import getWidgetDefinition
	from "../../../layout-content-page-editor-web/main/utils/getWidgetDefinition";
...
test.describe('Minium Theme Account Selector', async () => {
	test(
		...
		async ({
		   ...
		   site,
			...
			await test.step('Create an Account and a buyer', async () => {
				...
				const accountRoleBuyer = rolesResponse?.items?.filter(
					(role) => {
			...
			await test.step(
				'Create an order via API and login as a Buyer', async () => {
					order1 =
			...
			await test.step(
				'Go to Site and check that the account selector has the order selected',
				async () => {
			...
			await test.step(
				'Create an order from the create new order button and verify that the new order is selected',
				async () => {
			...
			await test.step(
				'Login as Admin and go to channel and change the max open order account',
				async () => {
					await performLogout(page);
			...
			await test.step(
				'Go to Minium site and assert that the create new order button is disabled',
				async () => {
