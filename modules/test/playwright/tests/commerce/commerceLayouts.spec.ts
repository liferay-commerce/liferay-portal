...
test('LPD-32237 Order actions and redirect fragments', async ({
	...
	try {
		...
		await commerceLayoutsPage.addFragment('Redirect', 'Order');
		...
		await expect(
			page.getByText(
				'This component redirects the user after an action is performed. For example, when a user clicks on the checkout button.'
			)
		).toBeVisible();
