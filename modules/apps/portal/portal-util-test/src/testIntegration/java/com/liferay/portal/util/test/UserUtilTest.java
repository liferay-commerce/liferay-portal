...
public class UserUtilTest {
	...
	public void testGetAllUserRoles() throws Exception {
		List<Role> roles = UserUtil.getAllUserRoles(_user);

		Assert.assertEquals(roles.toString(), 1, roles.size());

		_setRole();

		roles = UserUtil.getAllUserRoles(_user);
	...
	public void testGetInheritedRoles() throws Exception {
		List<Role> roles = UserUtil.getInheritedRoles(_user);

		Assert.assertEquals(roles.toString(), 0, roles.size());

		_setInheritedRole();

		roles = UserUtil.getInheritedRoles(_user);
	...
	public void testGetInheritedSiteRoles() throws Exception {
		List<Role> roles = UserUtil.getInheritedSiteRoles(_user);

		Assert.assertEquals(roles.toString(), 0, roles.size());

		_setInheritedSiteRole();

		roles = UserUtil.getInheritedSiteRoles(_user);
	...
	public void testGetOrganizationsRoles() throws Exception {
		List<Role> roles = UserUtil.getOrganizationsRoles(_user);

		Assert.assertEquals(roles.toString(), 0, roles.size());

		_setOrganizationRole();

		roles = UserUtil.getOrganizationsRoles(_user);
	...
	public void testGetSiteRoles() throws Exception {
		List<Role> roles = UserUtil.getSiteRoles(_user);

		Assert.assertEquals(roles.toString(), 0, roles.size());

		_setSiteRole();

		roles = UserUtil.getSiteRoles(_user);
