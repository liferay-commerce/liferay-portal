...
public class TalendDispatchTaskExecutorTest {
	...
	public void testExecute() throws Exception {
		...
		try {
			dispatchTrigger =
				_dispatchTriggerLocalService.updateDispatchTrigger(
					...
					"* * * * * *", 5, 5, 2024, 11, 11, false, false, 4, 4, 2024,
					0, 0);
		...
		Assert.assertEquals(dispatchLogs.toString(), 1, dispatchLogs.size());
	}

	private void _addTalendExecutableFileEntry(DispatchTrigger dispatchTrigger)
		throws Exception {

		Class<TalendDispatchTaskExecutorTest>
			talendDispatchTaskExecutorTestClass =
				TalendDispatchTaskExecutorTest.class;

		InputStream talendExecutableInputStream =
			talendDispatchTaskExecutorTestClass.getResourceAsStream(
				"commerce4956_0_1.zip");

		_dispatchFileRepository.addFileEntry(
			dispatchTrigger.getCompanyId(), dispatchTrigger.getUserId(),
			dispatchTrigger.getDispatchTriggerId(), "testTalendArchive.zip", 0,
			"application/zip", talendExecutableInputStream);
	}
