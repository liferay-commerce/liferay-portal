...
>
	<aui:form action="<%= editCommerceDiscountRuleActionURL %>" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + liferayPortletResponse.getNamespace() + "apiSubmit();" %>'>
			...
			<div class="col-12">
				...
				>
					...
					<aui:select disabled="<%= true %>" name="type" required="<%= true %>">
						...
						<%
						for (CommerceDiscountRuleType commerceDiscountRuleType : commercePromotionRuleDisplayContext.getCommerceDiscountRuleTypes()) {
		...
		<div class="row">
			...
			<%
			CommerceDiscountRuleTypeJSPContributor commerceDiscountRuleTypeJSPContributor = commercePromotionRuleDisplayContext.getCommerceDiscountRuleTypeJSPContributor(type);
			%>
			...
			return CommerceDiscountRuleResource.updateDiscountRule(
				...
				.then(function () {
					...
					window.parent.Liferay.fire(events.UPDATE_DATASET_DISPLAY, {
						id:
							'<%= CommercePricingDataSetConstants.COMMERCE_DATA_SET_KEY_PROMOTION_RULES %>',
					});
