...
CommerceDiscount commerceDiscount = commercePromotionDisplayContext.getCommerceDiscount();
long commerceDiscountId = commercePromotionDisplayContext.getCommerceDiscountId();

PortletURL portletDiscountRuleURL = commercePromotionDisplayContext.getPortletDiscountRuleURL();
...
String amountSuffix = HtmlUtil.escape(commercePromotionDisplayContext.getDefaultCommerceCurrencyCode());
...
boolean hasPermission = commercePromotionDisplayContext.hasPermission(ActionKeys.UPDATE);
%>
...
<aui:form action="<%= editCommerceDiscountActionURL %>" cssClass="pt-4" method="post" name="fm">
			...
			>
				...
				<div class="row">
					<div class="col-6">
						<aui:select label="type" name="usePercentage" onChange='<%= liferayPortletResponse.getNamespace() + "selectType();" %>' required="<%= true %>">
							...
							%>
								...
								<aui:option label="<%= commerceDiscountType %>" selected="<%= commercePromotionDisplayContext.getUsePercentage(commerceDiscountType) == commerceDiscount.isUsePercentage() %>" value="<%= commercePromotionDisplayContext.getUsePercentage(commerceDiscountType) %>" />
					...
					<div class="col-6">
						<aui:select label="apply-to" name="target" onChange='<%= liferayPortletResponse.getNamespace() + "selectTarget();" %>' required="<%= true %>">
							...
							<%
							for (CommerceDiscountTarget commerceDiscountTarget : commercePromotionDisplayContext.getCommerceDiscountTargets()) {
							%>
				...
				<div class="row">
					<div class="<%= colCssClass %>">
						<aui:input ignoreRequestValue="<%= true %>" name="amount" suffix="<%= amountSuffix %>" type="text" value="<%= commercePromotionDisplayContext.getCommerceDiscountAmount(locale) %>">
					...
					<c:if test="<%= usePercentage %>">
						<div class="<%= colCssClass %>">
							<aui:input ignoreRequestValue="<%= true %>" name="maximumDiscountAmount" suffix="<%= HtmlUtil.escape(commercePromotionDisplayContext.getDefaultCommerceCurrencyCode()) %>" type="text" value="<%= (commerceDiscount == null) ? BigDecimal.ZERO : commercePromotionDisplayContext.round(commerceDiscount.getMaximumDiscountAmount()) %>">
								<aui:validator name="min">0</aui:validator>
								<aui:validator name="number" />
							</aui:input>
						</div>
	...
	<div class="row">
		<div class="col-12">
			<c:if test="<%= commercePromotionDisplayContext.hasCustomAttributesAvailable() %>">
				<commerce-ui:panel
