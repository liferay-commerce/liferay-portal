...
@Generated("")
@GraphQLName(
	description = "Promotional discount program scoped to a single company. Backed by CommerceDiscount; carries the target slice (subtotal, total, products, categories, skus, product-groups, order-types, or shipping), the limitation policy that caps reuse, an optional couponCode gate, the percentage tiers (or fixed-amount cap), and the cascading rel collections (account-group, account, category, channel, order-type, product, product-group, sku, rule). Workflow-aware -- the runtime only honours discounts whose status is approved (WorkflowConstants.STATUS_APPROVED).",
	value = "Discount"
)
...
public class Discount implements Serializable {
	...
	@io.swagger.v3.oas.annotations.media.Schema(
		description = "Free-form Expando custom fields attached to the underlying CommerceDiscount entity. Keys are Expando attribute names; values follow each attribute's declared column type."
	)
	@Valid
	...
	@io.swagger.v3.oas.annotations.media.Schema(
		description = "Channel bindings cascaded on upsert. Each entry restricts the discount to one CommerceChannel; supplying this array on POST/PUT replaces the previous bindings."
	)
	@Valid
	...
	@io.swagger.v3.oas.annotations.media.Schema(
		description = "Order-type bindings cascaded on upsert. Each entry restricts the discount to one CommerceOrderType; supplying this array on POST/PUT replaces the previous bindings."
	)
	@Valid
	...
	@io.swagger.v3.oas.annotations.media.Schema(
		description = "CommerceProductGroup bindings cascaded on upsert. Each entry restricts the discount to one product group; supplying this array on POST/PUT replaces the previous bindings."
	)
	@Valid
...
// LIFERAY-REST-BUILDER-HASH:1726068313