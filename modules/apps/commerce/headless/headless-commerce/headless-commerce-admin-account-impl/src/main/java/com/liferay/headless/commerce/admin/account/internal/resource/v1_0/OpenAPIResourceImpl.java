...
@Generated("")
@OpenAPIDefinition(
	info = @Info(description = "Liferay Commerce Admin Account API. Manages B2B and B2C Commerce accounts and their relationships from the admin perspective. -- Primary entities -- Account (the commerce buyer organization), AccountGroup (collections of accounts that share rules), AccountMember (links users to accounts with account-scoped roles), AccountOrganization (links accounts to Liferay Organizations), AccountAddress (billing and shipping addresses), AccountChannelEntry (deprecated per-account overrides of channel-level rules including currency, delivery and payment terms, discounts, payment methods, price lists, and reserved shipping or billing addresses), and AccountChannelShippingOption (per-account overrides of available shipping methods on a channel). -- Perspective -- administrators with the CommerceOpenApiAdmin.read or CommerceOpenApiAdmin.write scope. Endpoints expose the full account graph, including HATEOAS action descriptors and custom fields, regardless of the requesting user's account membership. -- Related modules -- buyer-perspective endpoints live in com.liferay.headless.commerce.delivery.cart and com.liferay.headless.commerce.delivery.catalog. Channel and pricing configuration lives in com.liferay.headless.commerce.admin.channel and com.liferay.headless.commerce.admin.pricing. -- Common workflows -- (1) Provision an account, addresses, and members from the back-office. POST /accounts with externalReferenceCode upserts the account; nested addresses and members are persisted in the same call. Add further memberships through POST /accounts/by-externalReferenceCode/<externalReferenceCode>/accountMembers (existing user) or .../accountMembers/createUser (new user). (2) Link an existing user as an account member. POST /accounts/by-externalReferenceCode/<externalReferenceCode>/accountMembers with userId or userExternalReferenceCode and the desired AccountRoles. PATCH on the member endpoint replaces the user's role set on the account. (3) Override channel-level rules for one account. POST /accounts/<id>/account-channel-* (or the by-externalReferenceCode variant) creates an override that takes precedence over the channel default. The top-level /account-channel-* endpoints are deprecated -- prefer the nested forms scoped under an account. (4) Group accounts for bulk rule application. POST /accountGroups creates the group (upsert by externalReferenceCode); POST /accountGroups/by-externalReferenceCode/<externalReferenceCode>/accounts links an account into the group, and the DELETE counterpart unlinks it. A Java client JAR is available for use with the group ID 'com.liferay', artifact ID 'com.liferay.headless.commerce.admin.account.client', and version '4.0.43'.", license = @License(name = "Apache 2.0", url = "http://www.apache.org/licenses/LICENSE-2.0.html"), title = "Liferay Commerce Admin Account API", version = "v1.0")
)
@Path("/v1.0")
...
// LIFERAY-REST-BUILDER-HASH:-1341699447