...
@Generated("")
@GraphQLName(
	description = "Per-account override of a shipping option on a commerce channel. Pairs the owning account, the channel, a shipping method engine key, and a specific fixed option key; only one override may exist for a given account and channel.",
...
public class AccountChannelShippingOption implements Serializable {
	...
	@DecimalMin("0")
	@io.swagger.v3.oas.annotations.media.Schema(
		description = "Reference to the shipping method (FK identifier). On write the method is resolved from `shippingMethodKey` first and falls back to this identifier; the method must be active and registered on the same channel.",
		example = "30324"
	)
	...
	@io.swagger.v3.oas.annotations.media.Schema(
		description = "Engine key of the shipping method. On write the method is resolved by key first and falls back to `shippingMethodId`; the method must be active and registered on the same channel.",
		example = "fixed"
	)
	...
	@DecimalMin("0")
	@io.swagger.v3.oas.annotations.media.Schema(
		description = "Reference to the fixed shipping option (FK identifier). Used when the resolved shipping method has fixed options; the option must be registered on the resolved shipping method.",
		example = "30324"
	)
	...
	@io.swagger.v3.oas.annotations.media.Schema(
		description = "Engine key of the fixed shipping option. On write the option is resolved by key first and falls back to `shippingOptionId`; the option must be registered on the resolved shipping method.",
		example = "expedited"
	)
...
// LIFERAY-REST-BUILDER-HASH:-1663327586