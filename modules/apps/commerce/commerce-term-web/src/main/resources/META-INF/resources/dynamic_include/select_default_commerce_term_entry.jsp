...
>
	...
	<aui:form action="<%= editAccountEntryDefaultCommerceTermEntryActionURL %>" method="post" name="fm">
		...
		<aui:select label="channel" name="commerceChannelId">
			<aui:option label='<%= LanguageUtil.get(request, "all-channels") %>' selected="<%= commerceChannelAccountEntryRelDisplayContext.isCommerceChannelSelected(0) %>" value="0" />
		...
		<aui:select label="term" name="classPK">

			<%
			for (CommerceTermEntry commerceTermEntry : commerceChannelAccountEntryRelDisplayContext.getCommerceTermEntries()) {
			%>

				<aui:option label="<%= commerceTermEntry.getName() %>" selected="<%= commerceChannelAccountEntryRelDisplayContext.isCommerceTermEntrySelected(commerceTermEntry.getCommerceTermEntryId()) %>" value="<%= commerceTermEntry.getCommerceTermEntryId() %>" />
