...
<aui:script require="commerce-frontend-js/utilities/eventsDefinitions as events, commerce-frontend-js/ServiceProvider/index as ServiceProvider">
	Liferay.provide(
		...
		() => {
			...
			var orderTypeId = form.querySelector('#<portlet:namespace />commerceOrderTypeId').value;

			var CartResource = ServiceProvider.default.DeliveryCartAPI('v1');

			CartResource.createCartByChannelId('<%= commerceChannel.getCommerceChannelId() %>', {
				accountId: '<%= commerceOrderContentDisplayContext.getCommerceAccountId() %>',
				cartItems: [{
					options: '<%= ParamUtil.getString(PortalUtil.getOriginalServletRequest(request), "options") %>',
					quantity: '<%= ParamUtil.getString(PortalUtil.getOriginalServletRequest(request), "quantity") %>',
					skuId: '<%= ParamUtil.getString(PortalUtil.getOriginalServletRequest(request), "skuId") %>',
				}],
