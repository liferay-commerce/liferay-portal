...
public class OrderSummaryFragmentRenderer implements FragmentRenderer {
	...
	private String _getFieldValue(
		...
		if (field.equals("subtotalFormatted")) {
			return _getPriceFormatted(
				commerceOrder, commerceOrder.getSubtotal(), locale);
		}
		else if (field.equals("subtotalDiscountValueFormatted")) {
			return _getPriceFormatted(
				commerceOrder, commerceOrder.getSubtotalDiscountAmount(),
				locale);
		}
		else if (field.equals("totalDiscountValueFormatted")) {
			return _getPriceFormatted(
				commerceOrder, commerceOrder.getTotalDiscountAmount(), locale);
		}
		else if (field.equals("couponCode")) {
			return commerceOrder.getCouponCode();
		}
		else if (field.equals("taxValueFormatted")) {
			return _getPriceFormatted(
				commerceOrder, commerceOrder.getTaxAmount(), locale);
		}
		else if (field.equals("shippingValueFormatted")) {
			return _getPriceFormatted(
				commerceOrder, commerceOrder.getShippingAmount(), locale);
		}
		else if (field.equals("shippingDiscountValueFormatted")) {
			return _getPriceFormatted(
				commerceOrder, commerceOrder.getShippingDiscountAmount(),
				locale);
		}
		else if (field.equals("totalFormatted")) {
			return _getPriceFormatted(
				commerceOrder, commerceOrder.getTotal(), locale);
		}

		return StringPool.BLANK;
	}

	private String _getPriceFormatted(
			CommerceOrder commerceOrder, BigDecimal price, Locale locale)
		throws PortalException {

		return _commercePriceFormatter.format(
			commerceOrder.getCommerceCurrency(), price, locale);
	}
