...
public class CPDefinitionOptionRelLocalServiceImpl
	...
	public Map<Long, List<Long>>
		...
		for (int i = 0; i < skuOptionJSONArray.length(); i++) {
			...
			CPDefinitionOptionRel cpDefinitionOptionRel =
				cpDefinitionOptionRelLocalService.
					fetchCPDefinitionOptionRelByKey(
						cpDefinitionId, skuOptionJSONObject.getString("key"));
			...
			JSONArray skuOptionValueJSONArray = _jsonHelper.getValueAsJSONArray(
				"skuOptionValues", skuOptionJSONObject);

			for (int j = 0; j < skuOptionValueJSONArray.length(); j++) {
				JSONObject skuOptionValueJSONObject =
					skuOptionValueJSONArray.getJSONObject(j);

				CPDefinitionOptionValueRel cpDefinitionOptionValueRel =
					_cpDefinitionOptionValueRelLocalService.
						fetchCPDefinitionOptionValueRel(
							cpDefinitionOptionRel.getCPDefinitionOptionRelId(),
							skuOptionValueJSONObject.getString("key"));
